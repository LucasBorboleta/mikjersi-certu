...
"""mikjersi_gui.py implements a GUI for the MIKJERSI boardgame."""
...
import mikjersi_rules as rules
...
class CanvasConfig:
    ...
    # Canvas x-y dimensions in cell units
    NX = 7 + 0.75
    NY = 7
    ...
    # Cell geometrical data
    CELL_VERTEX_COUNT = 4
    CELL_SIDE_ANGLE = 2*math.pi/CELL_VERTEX_COUNT
    CELL_SIDE = min(HEIGHT, WIDTH) / max(NX, NY)
    CELL_HALF_DIAG = CELL_SIDE*math.sqrt(2)/2

    # Cube (square) geometrical data
    CUBE_VERTEX_COUNT = 4
    CUBE_SIDE_ANGLE = math.pi/2
    CUBE_SIDE = CELL_SIDE*0.40
    CUBE_HALF_DIAG = CUBE_SIDE*math.sqrt(2)/2

    # Font used for text in the canvas
    FONT_FAMILY = 'Calibri'
    FONT_LABEL_SIZE = int(0.175*CELL_SIDE) # size for 'e5', 'f5' ...
    FONT_FACE_SIZE = int(0.25*CELL_SIDE)  # size for 'K', 'F' ...
...
class GraphicalCell:
    ...
    def __init__(self, cell, color):
        ...
        assert cell.name not in GraphicalCell.__name_to_cell
        assert color in CellColor

        self.name = cell.name
        self.position_uv = cell.position_uv
        self.reserve = cell.reserve
        self.index = cell.index
        self.color = color

        GraphicalCell.__name_to_cell[self.name] = self


    def __str__(self):
        return f"GraphicalCell({self.name}, {self.position_uv}, {self.reserve}, {self.index}, {self.color})"


    @staticmethod
    def get(name):
        return GraphicalCell.__name_to_cell[name]


    @staticmethod
    def init():
        if not GraphicalCell.__init_done:
            GraphicalCell.__create_cells()
            GraphicalCell.__create_all_sorted_cells()
            GraphicalCell.__init_done = True


    @staticmethod
    def show_all():
        for cell in GraphicalCell.__all_sorted_cells:
            print(cell)


    @staticmethod
    def __create_all_sorted_cells():
        for name in sorted(GraphicalCell.__name_to_cell.keys()):
            GraphicalCell.__all_sorted_cells.append(GraphicalCell.__name_to_cell[name])

        GraphicalCell.all = GraphicalCell.__all_sorted_cells


    @staticmethod
    def __create_cells():

        borders = ['a1', 'a2', 'a3', 'a4', 'a5']
        borders += ['e1', 'e2', 'e3', 'e4', 'e5']
        borders += ['b1', 'c1', 'd1']
        borders += ['b5', 'c5', 'd5']
        ...
        for cell in rules.Cell.all:

            if cell.reserve:
                color = CellColor.RESERVE

            elif cell.name in borders:
                color = CellColor.BORDER

            elif cell.name in darks:
                color = CellColor.DARK
            else:
                color = CellColor.LIGHT

            GraphicalCell(cell, color)
...
class GameGui(ttk.Frame):
    ...
    def __init__(self):
        ...
        self.__mikjersi_state = rules.JersiState(play_reserve=self.__play_reserve)
        self.__searcher = [None, None]

        self.__action_input = None
        self.__action_validated = False
        
        self.__turn_states.append(self.__mikjersi_state)
        ...
        self.__variable_log.set(f"mikjersi-certu version {rules.__version__} is ready !")
        self.__variable_summary.set("(c) 2021 Lucas Borboleta ; mikjersi software license : GNU GPL ; mikjersi rules license : CC-BY-NC-SA")
    ...
    def __create_cube_photos(self):
        if self.__cube_photos is None:
            ...
            cube_photo_width = int(CanvasConfig.CUBE_SIDE)
    ...
    def __command_update_turn(self, *_):
        try:
            ...
            if 0 <= turn_index < len(self.__turn_states):
                self.__mikjersi_state = self.__turn_states[turn_index]
                self.__draw_state()
    ...
    def __command_action_confirm(self):
        ...
        (self.__action_validated,
         message) = rules.Notation.validate_simple_notation(self.__action_input,
                                                                  self.__mikjersi_state.get_action_simple_names())
    ...
    def __command_start_stop(self):
        ...
        if self.__game_started:
           ...
           self.__variable_log.set("mikjersi stopped")
           self.__button_start_stop.configure(text="Start")
           self.__progressbar['value'] = 0.
    ...
    def __draw_all_cubes(self):
        ...
        cell_top =  self.__mikjersi_state.get_cell_top()
        cell_bottom =  self.__mikjersi_state.get_cell_bottom()

        for cell in rules.Cell.all:

            top_index = cell_top[cell.index]
            bottom_index = cell_bottom[cell.index]
    ...
    def __draw_all_cells(self):

        for cell in GraphicalCell.all:

            self.__draw_cell(position_uv=cell.position_uv,
                         fill_color=cell.color.value,
                         label=cell.name,
    ...
    def __draw_cell(self, position_uv, fill_color='', label='', reserve=False):
        ...
        cell_center = CanvasConfig.ORIGIN + CanvasConfig.CELL_SIDE*(u*CanvasConfig.UNIT_U + v*CanvasConfig.UNIT_V)

        if reserve:
            cell_center = cell_center + math.copysign(0.20*CanvasConfig.CELL_SIDE, v)*CanvasConfig.UNIT_V

        cell_data = list()

        for vertex_index in range(CanvasConfig.CELL_VERTEX_COUNT):
            vertex_angle = (1/2 + vertex_index)*CanvasConfig.CELL_SIDE_ANGLE

            cell_vertex = cell_center
            cell_vertex = cell_vertex + CanvasConfig.CELL_HALF_DIAG*math.cos(vertex_angle)*CanvasConfig.UNIT_X
            cell_vertex = cell_vertex + CanvasConfig.CELL_HALF_DIAG*math.sin(vertex_angle)*CanvasConfig.UNIT_Y

            cell_data.append(cell_vertex[0])
            cell_data.append(cell_vertex[1])

            if vertex_index == 2:
                label_position = (cell_vertex +
                                  0.15*CanvasConfig.CELL_SIDE*(CanvasConfig.UNIT_X + 0.75*CanvasConfig.UNIT_Y))


        if reserve:
            polygon_line_color = CellLineColor.RESERVE.value
        else:
            polygon_line_color = CellLineColor.NORMAL.value

        self.__canvas.create_polygon(cell_data,
                              fill=fill_color,
                              outline=polygon_line_color,
                              width=CanvasConfig.CELL_LINE_WIDTH,
                              joinstyle=tk.MITER)
    ...
    def __draw_cube(self, name, config, cube_color, cube_sort, cube_label):
        ...
        cell = GraphicalCell.get(name)

        if cell.reserve and not self.__play_reserve:
            return

        (u, v) = cell.position_uv
        ...
        for vertex_index in range(CanvasConfig.CUBE_VERTEX_COUNT):
            ...
            if config == CubeLocation.MIDDLE:
                cube_center = cell_center

            elif config == CubeLocation.BOTTOM:
                cube_center = cell_center - 0.225*CanvasConfig.CELL_SIDE*CanvasConfig.UNIT_Y
                
            elif config == CubeLocation.TOP:
                cube_center = cell_center + 0.225*CanvasConfig.CELL_SIDE*CanvasConfig.UNIT_Y

            cube_vertex = cube_center
            cube_vertex = cube_vertex + CanvasConfig.CUBE_HALF_DIAG*math.cos(vertex_angle)*CanvasConfig.UNIT_X
            cube_vertex = cube_vertex + CanvasConfig.CUBE_HALF_DIAG*math.sin(vertex_angle)*CanvasConfig.UNIT_Y
